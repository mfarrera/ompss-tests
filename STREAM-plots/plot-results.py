...
def main():
  ...
  match = re.findall(r'Triad:\s+(\d+)\s+(\d+\.\d+)\s*.*',s)
  if not match:
    print ('did not find')
    sys.exit(0)
...
## Rate in MB/s
  throughputlistompss=[float(x[1]) for x in match]
  print (blocksizelist)
  ...
  s = f.read()
  #match = re.search(r'Triad:\s*',s)
  match = re.search(r'Triad:\s+(\d+\.\d+)\s*.*',s)
  if not match:
    print ('did not find')
    sys.exit(0)

## Rate in MB/s
  throughputlistserial=[float(match.group(1)) for x in blocksizelist]
...
## Rate in MB/s
  throughputlistopenmp=[float(match.group(1)) for x in blocksizelist]
...
#plt.subplot(211)
  ...
  plt.ylabel('Memory Throughput (MB/s)')
