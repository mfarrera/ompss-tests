...
static double ops[4] = {
    0 * sizeof(STREAM_TYPE) * N,
    1 * sizeof(STREAM_TYPE) * N,
    1 * sizeof(STREAM_TYPE) * N,
    2 * sizeof(STREAM_TYPE) * N
    };
...
//	for (j=0; j<4; j++)
    ...
    printf("Function  BlockSize  Best Rate MB/s  Avg time     Min time     Max time    MFlop/s    Flop/Byte\n");
...
//    for (j=0; j<4; j++) {
	...
	printf("%s%lu  %12.1f  %11.4f  %11.4f  %11.4f %12.1f  %11.6f\n", label[j], BSIZE*sizeof(double),
